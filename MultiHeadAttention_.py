...
class MultiheadAttention_(Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None,
        ...
        # type: (Tensor, Tensor, Tensor, Optional[Tensor], bool, Optional[Tensor], Optional[bool]) -> Tuple[Tensor, Optional[Tensor]]
        r"""
    Args:
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            if self.heads_weight is not None:
                self.heads_weight = self.softmax(self.heads_weight)*self.num_heads
            
            return multi_head_attention_forward_(
    ...
    if not torch.jit.is_scripting():
        ...
        if any([type(t) is not Tensor for t in tens_ops]) and has_torch_function(tens_ops):
            return F.handle_torch_function(
                multi_head_attention_forward_, tens_ops, query, key, value,
